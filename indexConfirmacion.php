...
$stmt = $db->prepare("SELECT * FROM [confirmaciones] WHERE " . $column . " LIKE :search AND " . $column2 . " LIKE :search2 AND " . $column3 . " LIKE :search3");
$stmt->execute(array(':search' => '%' . $search . '%', ':search2' => '%' . $search2 . '%', ':search3' => '%' . $search3 . '%'));
$data = $stmt->fetchAll(PDO::FETCH_ASSOC);

/// Preparar la consulta SQL
$stmt = $db->prepare("SELECT * FROM [confirmaciones]");
$stmt->execute();
...
file_put_contents('respaldos monterrey/respaldo_confirmaciones.txt', $respaldo);
}
// insertar respaldo
// Comprueba si el archivo de respaldo existe
if (isset($_GET['accion']) && $_GET['accion'] == 'leer_respaldo') {
// Comprueba si el archivo de respaldo existe
if (file_exists('respaldos monterrey/respaldo_confirmaciones.txt')) {
    // Lee las líneas del archivo de respaldo
    $lineas = file('respaldos monterrey/respaldo_confirmaciones.txt');
...
<div class="header">
    ...
    if ($user && ($user['es_administrador'] || $user['es_maestro'])) {
        ...
        echo ' ';
        echo '<button class="botonInsertar" onclick="window.location.href=\'indexConfirmacion.php?accion=leer_respaldo\'">Leer Respaldo</button>';
    }
    ?>
    ...
    function hacerRespaldo() {
        ...
        <?php
        $stmt = $db->prepare("SELECT COUNT(*) FROM [confirmaciones]");
        $stmt->execute();
        $count = $stmt->fetchColumn();
        ?>
        if (<?php echo $count; ?> == 0) {
            ...
            window.location.href = 'indexConfirmacion.php?accion=hacer_respaldo';
        }
    }
    </script>
            ...
            <td>
                ...
                if (isset($_SESSION['nombre_usuario'])) {
                    ...
                    if ($user['es_administrador']) {
                        echo '<a href="documentoConfirmacion.php?id=' . $row['id'] . '"><button type="button">' . $row['id'] . '</button></a>';
                        ...
                        echo '<a href="actualizarConfirmacion.php?id=' . $row['id'] . 
