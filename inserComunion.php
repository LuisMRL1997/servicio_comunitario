<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        empty($_POST['numLibro']) || empty($_POST['folio']) || empty($_POST['numeral'])) {
            ...
            $stmt = $db->prepare('INSERT INTO "comuniones" (
                "primer_nombre", 
                "segundo_nombre", 
                "primer_apellido", 
                "segundo_apellido", 
            ...
            if ($stmt->execute()) {
                echo 'Primera Comunion agregado exitosamente!';
            } else {
                echo 'Hubo un error al agregar la Primera Comunion.';
            }
        }
    }
...
<div class="header">
    ...
    <form class="formulario-estilizado" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
        ...
        <input type="submit" value="Agregar Comunion">
    </form>
    <button id="botonRegreso" onclick="location.href='indexComunion.php'">Volver</button>
    </div>
</body>
</html>