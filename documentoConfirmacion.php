...
<html>
<head>
    <title>Registros de Confirmaciones</title>
...
<div class="header">
    ...
    <?php
        if (isset($user) && ($user['es_maestro'] || $user['es_administrador'])) {
            echo '<button id="cerrarSesion" onclick="location.href=\'inserConfirmacion.php\'">Insertar Confirmacion</button>';
        }
        ?>
        </div>
    <div class="container">
        ...
        if (isset($_GET["id"])) {
            $id = $_GET["id"];
            $sql = "SELECT COUNT(*) FROM [confirmaciones] WHERE [id] = $id";
            $count = $db->query($sql)->fetchColumn();
            
            if ($count > 0) {
                $sql = "SELECT * FROM [confirmaciones] WHERE [id] = $id";
                $result = $db->query($sql);
                ...
                echo "<div id='content-to-capture'>";
                    echo "<div style='display: flex; align-items: flex-start; margin-left: 50px;'>"; // Alinea el texto con la parte superior de la imagen
                        ...
                        echo "</div>";
                    echo "</div>";
                echo "<h2>CERTIFICADO DE CONFIRMACION</h2>";   
                echo "<p style='text-indent: 50px; text-align: left; text-indent: 70px; margin-left: 20px; line-height: 1.5; padding: 20px;'>Quien suscribe, Presbitero Joel Javier Escalante Buitrago, en calidad de párroco de esta parroquia, certifica y da fe de que el ciudadano(a)</p>";
                echo "<p style='text-align: left; margin-left: 180px;'>  " . $row["primer_nombre"]. " " . $row["segundo_nombre"]. " " . $row["primer_apellido"]. " " . $row["segundo_apellido"]. "</p>";
                echo "<p style='text-align: left; margin-left: 180px;'>Fue bautizado (a) el día " . $row["dia_de_bautizo"]. " de " . numeroAMes(trim($row["mes_de_bautizo"])). " del año " . $row["ano_de_bautizo"]. ".</p>";
                echo "<p style='text-align: left; margin-left: 180px;'>Nació el día " . $row["dia_de_nacimiento"]. " de " . numeroAMes(trim($row["mes_de_nacimiento"])). " del año " . $row["ano_de_nacimiento"]. " en " . $row["lugar_de_nacimiento"]. " Estado " . $row["estado_de_nacimiento"]. ".</p>";
                echo "<div style='display: flex; justify-content: left; margin-left: 250px;'>
                ...
                echo "<div style='display: flex; justify-content: left; margin-left: 230px;'>
                        <div style='display: flex; align-items: center;'>
                ...
                echo "<div style='display: flex; justify-content: left; margin-left: 240px;'>
                        <div style='display: flex; align-items: center;'>
                ...
                echo "<p style='margin-left: 120px;'>Registrado en el Libro de Confirmacion Nº: " . $row["numLibro"]. " Folio: " . $row["folio"]. " Numeral: " . $row["numeral"]. ".</p>";
                        ...
                        document.getElementById('download').addEventListener('click', function() {
                            ...
                            domtoimage.toPng(node)
                                .then(function (dataUrl) {
                                    ...
                                    link.download = 'Confirmacion de " . $row["primer_nombre"] . " " . $row["segundo_nombre"] . " " . $row["primer_apellido"] . " " . $row["segundo_apellido"] . ".png';
                                    link.href = dataUrl;
                                    link.click();
                                })
    ...
    <a href="indexConfirmacion.php"><button class="button">Volver</button></a>
    </div>
</body>
</html>